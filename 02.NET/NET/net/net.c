...
unsigned char MQTT_FillBuf(char *buf)
{
	...
	strcat(buf, text);
	memset(text, 0, sizeof(text));
	sprintf(text, "\"humi_v\":\"%d\",",threshold_value_init.humi_value); 
	strcat(buf, text);
	
	memset(text, 0, sizeof(text));
	sprintf(text, "\"led\":\"%d\",", (u8 )relay2in);
	strcat(buf, text);
	...
	memset(text, 0, sizeof(text));
	sprintf(text, "\"soil_humi\":\"%d\",", Data_init.soil_humi); 
	strcat(buf, text);
	
	memset(text, 0, sizeof(text));
	sprintf(text, "\"soil_humi_v\":\"%d\",", threshold_value_init.soil_humi_value); 
	strcat(buf, text);
	
	memset(text, 0, sizeof(text));
	sprintf(text, "\"pump\":\"%d\",", (u8 )relay1in); 
	strcat(buf, text);
